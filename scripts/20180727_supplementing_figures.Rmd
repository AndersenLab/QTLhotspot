...
source_data("https://github.com/AndersenLab/QTLhotspot/blob/master/data/FileS6_correlated_traits.Rda?raw=TRUE")
source_data("https://github.com/AndersenLab/QTLhotspot/blob/master/data/FileS7_uniqueQTL.Rda?raw=TRUE")
...
#Split trait into condition and trait
splitmap <- allAnnotatedLods %>%
    dplyr::mutate(condition = stringr::str_split_fixed(trait, "\\.", 2)[,1],
                  trait = stringr::str_split_fixed(trait, "\\.", 2)[,2])

#First find all correlation coefficients
...
#Filter traits we are interested in and prepare for finding overlap - keep all QTL (not traits)
peaks <- splitmap %>%
    ...
    dplyr::mutate(drugtrait = paste(condition, trait, sep = "."), 
                  qtl = paste(marker, drugtrait, sep = "."))

#Make dataframe that has for each pair of overlapped traits what is their correlation
groupedoverlaps <- data.frame(corrcoeff = NA, VE = NA, trait1 = NA, trait2 = NA, chrtrait1 = NA, chrtrait2 = NA)
    ...
    for(j in (i+1):nrow(peaks)) {
        if(peaks$chr[i] == peaks$chr[j] &&
           ...
           peaks$condition[j] == peaks$condition[i]) {
            ...
            VE <- min(peaks$var_exp[i], peaks$var_exp[j]) # why is this the min?
            trait1 <- peaks$drugtrait[i]
            trait2 <- peaks$drugtrait[j]
            chrtrait1 <- peaks$qtl[i]
            chrtrait2 <- peaks$qtl[j]
            all <- c(corr, VE, trait1, trait2, chrtrait1, chrtrait2)
            groupedoverlaps <- rbind(groupedoverlaps, all)
        }
    }
}

groupedoverlaps <- groupedoverlaps %>%
    na.omit()

groupedoverlaps$corrcoeff <- as.numeric(groupedoverlaps$corrcoeff)
groupedoverlaps$VE <- as.numeric(groupedoverlaps$VE)

VE95th <- stats::quantile(groupedoverlaps$VE, .95)

overlapthresh <- groupedoverlaps %>%
...
**Figure S5** NIL and CSS trait categorizations. (**A**) Flowchart for categorizing traits from the NIL or CSS phenotyping assays. (**B**) Six categories across both NIL and CSS assays for traits on chromosome V. The miscellaneous category is not depicted, but encompasses any other combination of NIL and CSS assay results.


```{r s6,warning=F, message=F, echo=F,comment="K",results='asis',fig.width=7.5, fig.height=6}
# source_data("https://github.com/AndersenLab/QTLhotspot/blob/master/data/FileS6_correlated_traits.Rda?raw=TRUE")
# source_data("https://github.com/AndersenLab/QTLhotspot/blob/master/data/FileS11_assays_category.Rda?raw=TRUE")
# source_data("https://github.com/AndersenLab/QTLhotspot/blob/master/data/FileS3_allAnnotatedLods.Rda?raw=TRUE")
...
group_represent <- correlated_traits %>%
    dplyr::mutate(drugtrait = paste0(marker, ".", condition, ".", trait)) %>%
    dplyr::group_by(representative_trait) %>%
    dplyr::mutate(num_traits = n()) %>%
    dplyr::filter(num_traits > 1)
...
cortrtcat <- correlated_traits %>%
    dplyr::left_join(assays_category) %>%
    dplyr::select(chr, condition, trait, representative_trait, representative_qtl, trait_correlation, exp, primary_category) %>%
    dplyr::filter(!is.na(primary_category), exp != "CSSV") %>%
    dplyr::mutate(marker = stringr::str_split_fixed(representative_qtl, "\\.", 2)[,1],
                  representative_QTL = paste0(representative_trait, " (", exp, ")")) %>%
    dplyr::group_by(representative_QTL, exp) %>%
    dplyr::mutate(num_traits = n()) %>%
    dplyr::filter(num_traits > 1) %>%
    dplyr::group_by(representative_QTL, primary_category, exp) %>%
    dplyr::mutate(num_cat = n()) %>%
    dplyr::mutate(perc_cat = num_cat / num_traits *100) %>%
    dplyr::arrange(desc(representative_QTL), desc(exp)) %>%
    dplyr::distinct(representative_QTL, exp, primary_category, .keep_all = T)
cortrtcat$representative_QTL <- factor(cortrtcat$representative_QTL, levels = unique(cortrtcat$representative_QTL))
...
# stacked barplot
corgroupplot <- ggplot2::ggplot(data = cortrtcat, ggplot2::aes(x = representative_QTL, y = perc_cat)) +
  ...
  ggplot2::theme(axis.text.x = ggplot2::element_text(vjust = 0.5), 
                 axis.text = ggplot2::element_text(size = 10, face = "bold", color = "black"),
                 axis.title = ggplot2::element_text(size = 12, face = "bold", color = "black")) +
  ggplot2::labs(y = "Percent of traits", x = "Representative QTL") +
  ggplot2::coord_flip()
print(corgroupplot)
