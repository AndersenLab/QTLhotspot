...
load("~/Dropbox/AndersenLab/QTLpaper/data/final/FileS3_allRIAILregressed.RData")
...
quick_plot_V <- function(cond, pltrt, signif, ylab = pltrt){
  ...
  if(is.null(signif)) {
    test <- phen_gen %>%
      ggplot(.)+
      aes(x = factor(strain, levels = strain, labels = strain, ordered = T), y = phenotype,fill=strain)+
